...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Modifying
    @Query("UPDATE Report r SET r.volunteer = :volunteer, r.status = :assigned WHERE r.id = :id AND r.status = :expected")
    int assignIfStatus(@Param("id") Long id,
                       @Param("volunteer") Volunteer volunteer,
                       @Param("assigned") Status assigned,
                       @Param("expected") Status expected);
